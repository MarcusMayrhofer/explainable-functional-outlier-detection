...
# #COVARIANCE MATRICES
# Sigma1 <- cellWise::generateCorMat(d = p, corrType = "A09")
# Sigma2 <- cellWise::generateCorMat(d = p, corrType = "ALYZ")

# plot(mu1.0(seq(0,1,length.out = 100), p = 1), type = "l")
# lines(mu1.2(seq(0,1,length.out = 100), p = 1), type = "l")
# lines(mu1.0(seq(0,1,length.out = 100), p = 1) + eigen(cov_col)$vectors[,10]*5, type = "l")
...
res <-
  ...
  foreach(i3 = 1:length(p_vec), 
          .combine = "rbind", 
          .packages = c("robustmatrix", "mrfDepth", "fdaoutlier", "fda", "dplyr", "tidyr", "tibble", "rSPDE", "reticulate"), 
    ...
    # Sigma_outlier <- cellWise::generateCorMat(d = p, corrType = "ALYZ")
    Sigma_outlier <- Sigma
    ...
    # plot(eigen(cov_col)$vectors[,1], type = "l")
...
close(pb)
stopCluster(cl)
save("res",file = "raw_results/simulation_res_p50_shift_FIF.RData")


# load("simulation_res_eigen_shift_M_01.RData")

# ribbon_fun <- function(y) {(c("y" = mean(y), 
#                               "ymin" = mean(y) - sd(y)/sqrt(length(y)),
#                               "ymax" =mean(y) + sd(y)/sqrt(length(y))))}
# p1 <- ggplot(res_eigen_shift_M %>% drop_na(), aes(x = eigen_shift_vec, y = F.score, color = method, group = method)) + 
#   # geom_smooth() + 
#   stat_summary(geom="line", fun.data = ribbon_fun, size = 0.4, 
#                color = "#4b4686", show.legend = FALSE, linetype = 3) +
#   # stat_summary(geom="ribbon", fun.data = ribbon_fun, alpha = 0.2, linetype = 2, size = 0.05) +
#   facet_grid(~eigen_id)#+                             
# # ylim(c(0,100))
# p1
# plotly::ggplotly(p1)
