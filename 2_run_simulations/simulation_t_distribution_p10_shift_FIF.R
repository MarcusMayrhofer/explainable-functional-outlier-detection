...
p_vec = c(10)
dim_vec <- c(1000)
eigen_shift_vec <- c(1,10) 
eigen_id_vec <- c(1)
cov_function_type = c("M")
eps_coord_vec <- c(0.1,0.5,1)
df_vec <- c(3,5,8,12,15)
lambda_vec <- NA
kappa_vec <- NA
nu_vec <- NA
...
npar <- n_run*length(p_vec)*length(eps_vec)*length(dim_vec)*length(eigen_shift_vec)*
  ...
  length(eps_coord_vec)*length(lambda_vec)*length(kappa_vec)*length(nu_vec)*length(df_vec)
...
res <-
  ...
  foreach(i13 = 1:length(df_vec), .combine = "rbind") %:%
  foreach(i3 = 1:length(p_vec), .combine = "rbind", .packages = c("robustmatrix", "mrfDepth", "fdaoutlier", "fda", "dplyr", "tidyr", "tibble", "rSPDE"), .options.snow = opts) %dopar% {
    ...
    create_data <- msp(from = from, to = to, p = p, q = q, n = n, 
                       mu = mu_function, cov_row = Sigma, K = K, type = "t", df = df)
    mu = create_data$mu
    ...
    if(outlier_percentage > 0){
      ...
      if(!is.na(eigen_id) | !is.na(eigen_shift)){
        create_data_outlier <- msp(from = from, to = to, p = p, q = q, n = n,
                                   ...
                                   K = K_outlier, mu_rand = TRUE, eigen_id = eigen_id, gamma = eigen_shift, type = "t", df = df)
      } else{
        create_data_outlier <- msp(from = from, to = to, p = p, q = q, n = n,
                                   mu = mu_function_outlier, cov_row = Sigma_outlier, 
                                   K = K_outlier, mu_rand = TRUE, type = "t", df = df)
      }
...
close(pb)
stopCluster(cl)
save("res",file = "raw_results/simulation_res_p10_t_distribution_shift_FIF.RData")
